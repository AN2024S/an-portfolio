...
export const ProjectDetailsModal = ({ selectedProject, darkMode, onClose, onImageSelect, t }) => (
  <AnimatePresence>
    {selectedProject && (
      <motion.div 
      ...
      >
        ...
        >
          {/* زر الإغلاق: قمنا بتغيير مكانه ليكون مرناً مع الاتجاه */}
          <button onClick={onClose} className="absolute top-6 start-6 p-3 hover:text-accent z-50 rounded-full bg-white/5">
            <X size={32} />
          </button>

          <div className="text-center mt-8 mb-12">
            <h3 className="text-4xl md:text-6xl font-black text-accent">{selectedProject.title}</h3>
          </div>

          <div className="flex flex-col gap-12 items-center">
            <div className="w-full max-w-3xl space-y-10">
              {/* استخدام t.problem و t.solution بدلاً من النصوص الثابتة */}
              <DetailBox title={t.problem} content={selectedProject.problem} />
              <DetailBox title={t.solution} content={selectedProject.solution} />
            </div>

            <div className={`w-full max-w-4xl p-8 rounded-3xl border ${darkMode ? 'bg-accent/5 border-accent/20' : 'bg-slate-50 border-slate-200'}`}>
              <h5 className="text-accent font-black text-xl mb-6">
                {/* يمكنك إضافة هذا النص لملف constants أو تركه هكذا */}
                {t.tech === 'التقنيات:' ? 'مثال من الكود النظيف' : 'Clean Code Example'}
              </h5>
            ...
            <div className="w-full max-w-4xl text-center">
              <h5 className="text-accent font-extrabold text-2xl mb-8">
                {t.viewGallery || "Project Images"}
              </h5>
...
export const LightboxModal = ({ selectedImage, galleryImages, currentImageIndex, pinchZoomRef, onUpdate, imgRef, onClose, setCurrentImageIndex }) => (
  <AnimatePresence>
    {selectedImage && (
      ...
      >
        {/* زر الإغلاق في الـ Lightbox */}
        <button onClick={onClose} className="absolute top-8 start-8 p-4 rounded-full z-[600] bg-white/10 text-white hover:bg-white/20 transition-all">
          <X size={35} />
        </button>
